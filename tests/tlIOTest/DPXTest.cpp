...
namespace tl
{
    namespace io_tests
    {
        DPXTest::DPXTest(const std::shared_ptr<ftk::Context>& context) :
            ITest(context, "io_tests::DPXTest")
        {}

        std::shared_ptr<DPXTest> DPXTest::create(const std::shared_ptr<ftk::Context>& context)
        {
        ...
        namespace
        {
                ...
                const Options& options)
            {
                ...
                FTK_ASSERT(videoData.image);                             FTK_ASSERT(videoData.image);
                    ...
                    for (const auto& size : sizes)
                    {
                        for (const auto pixelType : ftk::getImageTypeEnums())
                        {
                            for (const auto& option : options)
                            {
                                ...
                                const auto imageInfo = writePlugin->getInfo(ftk::ImageInfo(size, pixelType), options);
