...
namespace tl
{
    ...
    {
        MemoryReferenceTest::MemoryReferenceTest(const std::shared_ptr<ftk::Context>& context) :
            ITest(context, "timeline_tests::MemoryReferenceTest")
        {}

        std::shared_ptr<MemoryReferenceTest> MemoryReferenceTest::create(const std::shared_ptr<ftk::Context>& context)
        {
        ...
        {
            {
                ...
                FTK_ASSERT(v->memory() == memory.data());
                FTK_ASSERT(v->memory_size() == memory.size());
            }
            {
            ...
            }
            {
                ...
                FTK_ASSERT(v->memory() == memory);
                FTK_ASSERT(v->memory_sizes() == sizes);
            }
            {
