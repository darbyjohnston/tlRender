     ...
     const char *&outPtr)
{
    ...
    uint64_t counterBuf[NUM_SIZES_SINGLE];
    memcpy(counterBuf, inPtr, NUM_SIZES_SINGLE * sizeof(uint64_t));
    for (int i = 0; i < NUM_SIZES_SINGLE; ++i)
    {
        uint64_t   *dst =  counterBuf + i;
    ...
    const uint64_t *counterBufPtr64 = (const uint64_t*) counterBuf;

    uint64_t version                  = *(counterBufPtr64 + VERSION);
    uint64_t unknownUncompressedSize  = *(counterBufPtr64 + UNKNOWN_UNCOMPRESSED_SIZE);
    uint64_t unknownCompressedSize    = *(counterBufPtr64 + UNKNOWN_COMPRESSED_SIZE);
    uint64_t acCompressedSize         = *(counterBufPtr64 + AC_COMPRESSED_SIZE);
    uint64_t dcCompressedSize         = *(counterBufPtr64 + DC_COMPRESSED_SIZE);
    uint64_t rleCompressedSize        = *(counterBufPtr64 + RLE_COMPRESSED_SIZE);
    uint64_t rleUncompressedSize      = *(counterBufPtr64 + RLE_UNCOMPRESSED_SIZE);
    uint64_t rleRawSize               = *(counterBufPtr64 + RLE_RAW_SIZE);
 
    uint64_t totalAcUncompressedCount = *(counterBufPtr64 + AC_UNCOMPRESSED_COUNT); 
    uint64_t totalDcUncompressedCount = *(counterBufPtr64 + DC_UNCOMPRESSED_COUNT); 

    uint64_t acCompression            = *(counterBufPtr64 + AC_COMPRESSION); 
