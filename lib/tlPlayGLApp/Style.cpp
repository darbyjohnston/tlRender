// SPDX-License-Identifier: BSD-3-Clause
// Copyright (c) 2021-2023 Darby Johnston
// All rights reserved.

#include <tlPlayGLApp/Style.h>

#include <tlCore/Error.h>
#include <tlCore/String.h>

namespace tl
{
    namespace play_gl
    {
        TLRENDER_ENUM_IMPL(
            StylePalette,
            "Dark",
            "Light");
        TLRENDER_ENUM_SERIALIZE_IMPL(StylePalette);

        std::map<ui::ColorRole, imaging::Color4f> getStylePalette(StylePalette value)
        {
            auto out = ui::defaultColorRoles();
            switch (value)
            {
            case StylePalette::Light:
                out[ui::ColorRole::Window] = imaging::Color4f(.9F, .9F, .9F);
                out[ui::ColorRole::Base] = imaging::Color4f(1.F, 1.F, 1.F);
                out[ui::ColorRole::Button] = imaging::Color4f(.8F, .8F, .8F);
                out[ui::ColorRole::Text] = imaging::Color4f(0.F, 0.F, 0.F);
                out[ui::ColorRole::TextDisabled] = imaging::Color4f(.3F, .3F, .3F);
                out[ui::ColorRole::Border] = imaging::Color4f(.6F, .6F, .6F);
                out[ui::ColorRole::Hover] = imaging::Color4f(1.F, 1.F, 1.F, .5F);
                out[ui::ColorRole::Pressed] = imaging::Color4f(1.F, 1.F, 1.F, .9F);
                out[ui::ColorRole::Checked] = imaging::Color4f(.5F, .6F, 1.F);
                out[ui::ColorRole::KeyFocus] = imaging::Color4f(.1F, .2F, 1.F);

                out[ui::ColorRole::InOut] = imaging::Color4f(.4F, .5F, .9F);
                out[ui::ColorRole::VideoCache] = imaging::Color4f(.3F, .7F, .7F);
                out[ui::ColorRole::AudioCache] = imaging::Color4f(.5F, .3F, .7F);
                out[ui::ColorRole::VideoClip] = imaging::Color4f(.5F, .7F, .7F);
                out[ui::ColorRole::VideoGap] = imaging::Color4f(.55F, .61F, .61F);
                out[ui::ColorRole::AudioClip] = imaging::Color4f(.6F, .55F, .7F);
                out[ui::ColorRole::AudioGap] = imaging::Color4f(.55F, .54F, .6F);
                out[ui::ColorRole::Transition] = imaging::Color4f(.7F, .6F, .6F);
                break;
            default: break;
            }
            return out;
        }
    }
}
