    ...
    namespace qtwidget
    {
        namespace
        {
            ...
            public:
                ...
                {
                    IWindow::_init("tl::qtwidget::Window", context, nullptr);
                }
        ...
        struct TimelineWindow::Private
        {
            ...
            std::shared_ptr<qtwidget::Window> timelineWindow;
            ...
            _p(new Private)
        {
            ...
            p.timelineWindow = Window::create(context);
            ...
            connect(p.timer.get(), &QTimer::timeout, this, &TimelineWindow::_timerUpdate);
        ...
        void TimelineWindow::setPlayer(const std::shared_ptr<timeline::Player>& player)
        {
            _p->timelineWidget->setPlayer(player);
        }

        bool TimelineWindow::isEditable() const
        {
            return _p->timelineWidget->isEditable();
        }

        bool TimelineWindow::hasFrameView() const
        {
            return _p->timelineWidget->hasFrameView();
        }

        bool TimelineWindow::areScrollBarsVisible() const
        {
            return _p->timelineWidget->areScrollBarsVisible();
        }

        bool TimelineWindow::hasScrollToCurrentFrame() const
        {
            return _p->timelineWidget->hasScrollToCurrentFrame();
        }

        ui::KeyModifier TimelineWindow::scrollKeyModifier() const
        {
            return _p->timelineWidget->getScrollKeyModifier();
        }

        float TimelineWindow::mouseWheelScale() const
        {
            return _p->timelineWidget->getMouseWheelScale();
        }

        bool TimelineWindow::hasStopOnScrub() const
        {
            return _p->timelineWidget->hasStopOnScrub();
        }

        const std::vector<int>& TimelineWindow::frameMarkers() const
        {
            return _p->timelineWidget->getFrameMarkers();
        }

        const timelineui::ItemOptions& TimelineWindow::itemOptions() const
        {
            return _p->timelineWidget->getItemOptions();
        }

        void TimelineWindow::setEditable(bool value)
        {
            _p->timelineWidget->setEditable(value);
        }

        void TimelineWindow::setFrameView(bool value)
        {
            _p->timelineWidget->setFrameView(value);
        }

        void TimelineWindow::setScrollBarsVisible(bool value)
        {
            _p->timelineWidget->setScrollBarsVisible(value);
        }

        void TimelineWindow::setScrollToCurrentFrame(bool value)
        {
            _p->timelineWidget->setScrollToCurrentFrame(value);
        }

        void TimelineWindow::setScrollKeyModifier(ui::KeyModifier value)
        {
            _p->timelineWidget->setScrollKeyModifier(value);
        }

        void TimelineWindow::setMouseWheelScale(float value)
        {
            _p->timelineWidget->setMouseWheelScale(value);
        }

        void TimelineWindow::setStopOnScrub(bool value)
        {
            _p->timelineWidget->setStopOnScrub(value);
        }

        void TimelineWindow::setFrameMarkers(const std::vector<int>& value)
        {
            _p->timelineWidget->setFrameMarkers(value);
        }

        void TimelineWindow::setItemOptions(const timelineui::ItemOptions& value)
        {
            _p->timelineWidget->setItemOptions(value);
        }

        void TimelineWindow::initializeGL()
        {
            TLRENDER_P();
        ...
        void TimelineWindow::resizeGL(int w, int h)
        {
            TLRENDER_P();
            _setGeometry();
            p.vao.reset();
        ...
        void TimelineWindow::mousePressEvent(QMouseEvent* event)
        {
            TLRENDER_P();
            event->accept();
            int button = -1;
        ...
        void TimelineWindow::mouseReleaseEvent(QMouseEvent* event)
        {
            TLRENDER_P();
            event->accept();
            int button = 0;
        ...
        void TimelineWindow::mouseMoveEvent(QMouseEvent* event)
        {
            TLRENDER_P();
            event->accept();
        ...
        void TimelineWindow::wheelEvent(QWheelEvent* event)
        {
            TLRENDER_P();
        ...
        void TimelineWindow::keyReleaseEvent(QKeyEvent* event)
        {
            TLRENDER_P();
            if (p.timelineWindow->key(
                fromQtKey(event->key()),
        ...
        bool TimelineWindow::event(QEvent* event)
        {
            TLRENDER_P();
            bool out = QOpenGLWindow::event(event);
        ...
        void TimelineWindow::_tickEvent()
        {
            TLRENDER_P();
        ...
        bool TimelineWindow::_getSizeUpdate(const std::shared_ptr<ui::IWidget>& widget) const
        {
        ...
        void TimelineWindow::_sizeHintEvent()
        {
            TLRENDER_P();
            const float devicePixelRatio = this->devicePixelRatio();
        ...
        void TimelineWindow::_setGeometry()
        {
            TLRENDER_P();
        ...
        void TimelineWindow::_clipEvent()
        {
            TLRENDER_P();
            const math::Box2i geometry(0, 0, _toUI(width()), _toUI(height()));
        ...
        bool TimelineWindow::_getDrawUpdate(const std::shared_ptr<ui::IWidget>& widget) const
        {
            bool out = false;
            if (!widget->isClipped())
            {
        ...
        void TimelineWindow::_drawEvent(
            const std::shared_ptr<ui::IWidget>& widget,
        ...
        int TimelineWindow::_toUI(int value) const
        {
            const float devicePixelRatio = this->devicePixelRatio();
            return value * devicePixelRatio;
        }

        math::Vector2i TimelineWindow::_toUI(const math::Vector2i& value) const
        {
            const float devicePixelRatio = this->devicePixelRatio();
            return value * devicePixelRatio;
        }

        int TimelineWindow::_fromUI(int value) const
        {
            const float devicePixelRatio = this->devicePixelRatio();
            return devicePixelRatio > 0.F ? (value / devicePixelRatio) : 0.F;
        }

        math::Vector2i TimelineWindow::_fromUI(const math::Vector2i& value) const
        {
            const float devicePixelRatio = this->devicePixelRatio();
            return devicePixelRatio > 0.F ? (value / devicePixelRatio) : math::Vector2i();
        }

        void TimelineWindow::_timerUpdate()
        {
            if (_p)
            {
                _tickEvent();
        ...
        void TimelineWindow::_styleUpdate()
        {
            /*TLRENDER_P();
