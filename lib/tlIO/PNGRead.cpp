    ...
    namespace png
    {
        namespace
        {
            void pngMemoryRead(png_structp png_ptr, png_bytep outBytes, png_size_t byteCountToRead)
            {
                ...
                ftk::InMemoryFile* memory = static_cast<ftk::InMemoryFile*>(io_ptr);
                ...
                uint8_t& bitDepth)
            {
                ...
                *pngInfoEnd = png_create_info_struct(png);
                ...
                if (bitDepth >= 16 && ftk::Endian::LSB == ftk::getEndian())
                {
                    png_set_swap(png);
                }
            ...
            public:
                    ...
                    const ftk::InMemoryFile* memory)
                {
                    ...
                    ftk::ImageType pixelType = io::getIntType(channels, bitDepth);
                    if (ftk::ImageType::None == pixelType)
                    {
                        throw std::runtime_error(ftk::Format("Cannot open: \"{0}\"").arg(fileName));
                    }

                    _info = ftk::ImageInfo(width, height, pixelType);
                ...
                std::shared_ptr<ftk::Image> read()
                {
                    auto out = ftk::Image::create(_info);
