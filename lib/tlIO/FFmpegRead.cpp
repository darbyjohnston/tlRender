...
namespace tl
{
    namespace io
    {
        namespace ffmpeg
        {
                ...
                p.thread = std::thread(
                    [this, path]
                    {
                    ...
                    
                        {
                            ...
                            {
                                videoRequests.front()->promise.set_value(VideoData());
                                videoRequests.pop_front();
                            }
                            while (!audioRequests.empty())
                            {
                                audioRequests.front()->promise.set_value(AudioData());
            ...
            std::future<AudioData> Read::readAudio(const otime::TimeRange& time)
            {
                TLRENDER_P();
